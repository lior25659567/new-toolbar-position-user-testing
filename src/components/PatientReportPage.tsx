...
import BlockNavList, { BlockTypeIcon } from './report/BlockNavList';
import { BLOCK_TYPE_TINT } from './report/blockTheme';
...
import ReportWorkflowView from './report/ReportWorkflowView';
...
import heartlandLogo from '../assets/Report images/Heartland_Dental.jpg';
...
function TemplatePicker({ onSelect, selectedId, hideScratch }: {
  onSelect: (blocks: SupportedBlock[], name: string, id: string) => void;
  selectedId: string | null;
  /** Hide the "Start from Scratch" card (e.g. in the empty state, where the
   *  report is already scratch and the block quick-adds cover that path). */
  hideScratch?: boolean;
}) {
  return (
...
function TemplateCard({ name, description, onClick, isSelected, icon, demoId }: {
  name: string; description: string; onClick: () => void; isSelected: boolean; icon?: React.ReactNode; demoId?: string;
}) {
  const [hovered, setHovered] = useState(false);
  ...
  return (
    <button
      type="button"
      data-demo={demoId ? `template-card-${demoId}` : undefined}
      onClick={onClick}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
  ...
  return (
    <>
      ...
      <div
        ...
        <div data-demo="preview-scroll" style={{
          flex: 1,
          width: '100%',
          overflowY: 'auto',
          backgroundColor: 'var(--ads-background-inverse)',
...
// Common blocks surfaced as one-click quick-adds in the empty state (the
// fast "build it yourself" path alongside templates). Mirrors the add-block menu.
const QUICK_BLOCKS: { type: BlockType; label: string }[] = [
  { type: 'image', label: 'Image' },
  { type: 'comparison', label: 'Before / After' },
  { type: 'notes', label: 'Notes' },
  { type: 'cost-summary', label: 'Summary' },
  { type: 'rx', label: 'Prescription' },
  { type: 'next-appointment', label: 'Appointment' },
  { type: 'patient-instructions', label: 'Instructions' },
];

function QuickBlockChip({ type, label, onClick }: { type: BlockType; label: string; onClick: () => void }) {
  const [hovered, setHovered] = useState(false);
  const tint = BLOCK_TYPE_TINT[type] ?? { bg: color.neutral100, fg: color.textSubtle };
  return (
    <button
      type="button"
      data-demo={`empty-add-${type}`}
      onClick={onClick}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{
        ...
        border: `1px solid ${hovered ? 'var(--ads-border-accent-hover)' : color.borderDefault}`,
    ...
    >
      <span style={{
        ...
        <BlockTypeIcon type={type} size={15} />
      </span>
      <span style={{ fontSize: font.size.sm, fontWeight: font.weight.medium, color: color.textDefault }}>{label}</span>
    </button>
  );
}

function EmptyStatePlaceholder({ onTemplateSelect, selectedTemplateId, onAddBlock }: {
  onTemplateSelect: (blocks: SupportedBlock[], name: string, id: string) => void;
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: space[6] }}>
        {/* Hero — warm, specific, sets intent */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: space[1], textAlign: 'center' }}>
          <img src={reportEmptyStateImg} alt="" aria-hidden="true" style={{ width: 150, height: 'auto', marginBottom: space[3] }} />
          <div style={{ fontSize: font.size.lg, fontWeight: font.weight.medium, color: color.textHeading, letterSpacing: font.tracking.tight }}>
            Start this patient report
          </div>
          <div style={{ fontSize: font.size.sm, color: color.textSubtle, maxWidth: 420, lineHeight: font.lineHeight.normal }}>
            Begin with a template for a structured head start — or drop in a single block and build it your way. You can rearrange and add more anytime.
          </div>
        </div>

        {/* Primary path — templates */}
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: space[2] }}>
          ...
          <TemplatePicker onSelect={onTemplateSelect} selectedId={selectedTemplateId} hideScratch />
        </div>

        {/* Divider */}
        <div style={{ display: 'flex', alignItems: 'center', gap: space[3], width: '100%' }}>
          <div style={{ flex: 1, height: 1, background: color.borderDefault }} />
          <span style={{ fontSize: font.size.xs, color: color.textPlaceholder }}>or add a block</span>
          <div style={{ flex: 1, height: 1, background: color.borderDefault }} />
        </div>

        {/* Secondary path — one-click block quick-adds */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: space[2], justifyContent: 'center' }}>
          {QUICK_BLOCKS.map((b) => (
            <QuickBlockChip key={b.type} type={b.type} label={b.label} onClick={() => onAddBlock(b.type)} />
          ))}
        </div>
      </div>
    </div>
...
export interface PatientReportPageHandle {
  ...
  /** Read the current block array (for self-driving demos that edit blocks by id) */
  getBlocks: () => SupportedBlock[];
  /** Highlight + smooth-scroll a block by id (or clear with null) */
  setActiveBlock: (id: string | null) => void;
  /** Open/close the Templates disclosure panel */
  setTemplatesPanelOpen: (open: boolean) => void;
  /** Reset the report to a blank starting point (blank page, no template applied) */
  resetReport: () => void;
  /** Set the doctor signature (used by the demo to sign reliably) */
  setSignature: (url: string, method: 'draw' | 'upload' | 'saved') => void;
}
...
const PatientReportPage = forwardRef<PatientReportPageHandle, PatientReportPageProps>(function PatientReportPage({
  ...
  const [settings, setSettings] = useState<ReportSettings>(initialData?.settings ?? {
    ...
    doctorName: 'Dr. Smith',
    doctorImageUrl: '',
    clinicName: 'Heartland Dental',
    clinicLogoUrl: heartlandLogo,
    pinEnabled: false,
    pin: '',
    signatureUrl: '',
    signatureMethod: '',
  ...
  // Calm autosave indicator: flip to "Saving…" only briefly and at most once every
  // few seconds, so rapid edits (incl. the demo's char-by-char typing) don't make
  // it flicker between Saving/Saved every second.
  const saveTimer = useRef<number | undefined>(undefined);
  const lastSavingAt = useRef(0);
  const markSaving = () => {
    const now = Date.now();
    if (now - lastSavingAt.current < 4500) return; // recently shown — stay on "Saved"
    lastSavingAt.current = now;
    setSaved(false);
  ...
  useImperativeHandle(ref, () => ({
    setSettings: (s: ReportSettings) => { setSettings(s); markSaving(); },
    setPatient: (p: PatientInfo) => { setPatient(p); markSaving(); },
    setBlocks: (b: SupportedBlock[]) => { setBlocks(b); markSaving(); },
    setActiveTab,
    ...
    getBlocks: () => blocks,
    setActiveBlock: (id: string | null) => setActiveBlockId(id),
    setTemplatesPanelOpen: (open: boolean) => setTemplatesOpen(open),
    resetReport: () => {
      // Start from the default "new report" state — one empty image block —
      // rather than a fully empty editor.
      setBlocks([{ ...createImageBlock(), id: 'init-img' }]);
      setActiveBlockId(null);
      setAppliedTemplate(null);
      setSelectedTemplateId(null);
      setBlocksBeforeTemplate(null);
      setTemplatesOpen(false);
      // Clear patient + signature back to empty (keep clinic logo/name) so each
      // demo run starts with the empty placeholders, not leftovers from before.
      setPatient({ patientName: '', birthDate: '', chartNumber: '' });
      setSettings((s) => ({ ...s, signatureUrl: '', signatureMethod: '' }));
      setSaved(true);
    },
    setSignature: (url: string, method: 'draw' | 'upload' | 'saved') => {
  ...
  // Press "1" to toggle the node-based "workflow" view of the report.
  const [workflowMode, setWorkflowMode] = useState(false);
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key !== '1' || e.metaKey || e.ctrlKey || e.altKey) return;
  ...
  // "+ Add block" from the left nav — creates the block, appends it,
  // and focuses it.
  const handleAddBlock = (type: BlockType) => {
    const created = createBlock(type) as SupportedBlock;
    // Adding a block expands every block so the new content is visible.
    setBlocks((prev) => [...prev, created].map((b) => ({ ...b, collapsed: false }) as SupportedBlock));
    setActiveBlockId(created.id);
    markSaving();
  };

  // "Image gallery" from the Add Block menu — turns each selected gallery
  // image into its own image block, appended in a bunch.
  ...
  const handleInsertBlock = (type: BlockType, atIndex: number) => {
    ...
    setBlocks((prev) => {
      ...
      // Inserting a block expands every block so the new content is visible.
  ...
  if (workflowMode) {
    return (
      <ReportWorkflowView
        ...
        patient={patient}
        blocks={blocks}
        onExit={() => setWorkflowMode(false)}
        onAddBlock={handleAddBlock}
        onDeleteBlock={handleDeleteBlock}
        onOpenBlock={(id) => { setActiveBlockId(id); setWorkflowMode(false); }}
      />
    );
  }
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: space[3], flex: 1, minWidth: 0 }}>
          ...
          <InlineInput
            ...
            onChange={(v) => { setSettings((s) => ({ ...s, reportName: v })); markSaving(); }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: space[3], flexShrink: 0 }}>
          ...
          {/* Preview */}
          <SecondaryButton size={36} data-demo="preview" onClick={() => setPreviewOpen(true)}>
            Preview
          </SecondaryButton>

          {/* Share */}
          <PrimaryButton size={36} data-demo="share" onClick={() => handleExportOrShare('share')}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        >
          <div style={{
            ...
            width: '100%',
            margin: '0 auto',
            // Editor canvas: sections float as cards on the gray page — no
            // continuous white slab behind them.
            padding: `${space[5]} ${space[8]} ${space[12]}`,
            backgroundColor: 'transparent',
            minHeight: '100%',
            display: 'flex',
            flexDirection: 'column',
            ...
            />
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: blocks.length === 0 ? 'center' : 'flex-start' }}>
              {blocks.length === 0 ? (
            ...
            {/* Signature + powered-by footer share one card — only once the
                report has content; both are noise on a blank report. */}
            {blocks.length > 0 && (
            <div style={{
              backgroundColor: color.bgSurface,
              border: `1px solid ${color.borderDefault}`,
            ...
            {/* Powered-by — divided footer inside the same card */}
            <div style={{
