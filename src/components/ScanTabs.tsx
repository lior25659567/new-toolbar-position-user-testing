...
export default function ScanTabs({
  ...
  // Inline rename state for a scan tab. The committed label flows out via
  // onTabsChange, so the renamed layer also shows on the View page.
  ...
  const commitRename = () => {
    ...
    if (name) {
      const newTabs = tabs.map((t) => (t.id === editingTabId ? { ...t, label: name } : t));
      setTabs(newTabs);
      onTabsChange?.(newTabs);
    }
        ...
        return (
          <div
            ...
            style={{
              ...
              // Hug the label — no fixed min width, so short (e.g. renamed)
              // tabs collapse instead of leaving empty space. Caps at 240px
          ...
          >
            ...
            {editingTabId === tab.id ? (
              <input
                ...
                style={{
                  // Minimal in-place edit: a subtle gray "highlighted" field
                  // (design-system subtle surface) — no harsh borders, just a
                  // soft rounded fill sitting where the label was.
                  fontFamily: 'var(--ads-font-sans)',
                  fontSize: 'var(--tp-body-01-size)',
                  lineHeight: 'var(--tp-body-01-lh)',
                  fontWeight: 400,
                  color: 'var(--ads-text-primary)',
                  // Fixed, comfortable width while editing (the tab now hugs
                  // content, so a flex-grow field would collapse to nothing).
                  flex: '0 0 auto',
                  width: '140px',
                  maxWidth: '200px',
                  background: 'var(--ads-background-subtle-hover)',
                  border: 'none',
              ...
              <span
                ...
                onDoubleClick={(e) => {
                  if (tab.layerType !== 'additional-scan') return;
                  e.stopPropagation();
                  startRename(tab);
                }}
                title={tab.layerType === 'additional-scan' ? 'Double-click to rename' : undefined}
                style={{
                  ...
                  // Size to content (don't force-grow), so the tab hugs the
                  // name; only shrink/truncate when the 240px cap is hit.
                  flex: '0 1 auto',
                  minWidth: 0,
                }}
              >
            ...
            {/* Actions cluster — rename (Additional Scan only) + close, grouped
                in one container and set apart from the layer name. Shown on
                active / hover / focus, hidden while editing. */}
            {showClose && editingTabId !== tab.id && (tab.layerType === 'additional-scan' || canClose) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '2px', marginLeft: '12px', flexShrink: 0 }}>
                {tab.layerType === 'additional-scan' && (
                  <button
                    type="button"
                    tabIndex={-1}
                    onClick={(e) => {
                    ...
                    className="btn btn-ghost btn-sm flex items-center justify-center shrink-0 relative"
