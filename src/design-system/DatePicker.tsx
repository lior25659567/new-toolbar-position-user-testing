...
export interface DatePickerProps {
  ...
  placeholder?: string;
  showIcon?: boolean;
  /** Documentation-only: render the calendar open on mount (no click). */
  defaultOpen?: boolean;
  ...
  const dropdown = (
    <div ref={ref} style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
        <button type="button" onClick={prev} aria-label="Previous month" style={navBtnStyle}>
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden><path d="M10 4L6 8l4 4"/></svg>
        </button>
        <span style={{ fontSize: "13px", fontWeight: 600, color: color.textHeading, fontFamily: font.family }}>
          {MONTHS[viewMonth]} {viewYear}
        </span>
        <button type="button" onClick={next} aria-label="Next month" style={navBtnStyle}>
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden><path d="M6 4l4 4-4 4"/></svg>
        </button>
      </div>
...
export function DatePicker({
  ...
  // When opened (incl. defaultOpen on mount) without a click, capture the
  // trigger rect so the portal calendar anchors correctly instead of (0,0).
  React.useEffect(() => {
      ...
      <div style={{ position: "relative" }}>
        <button
          ...
          style={{
            ...
            border: `1px solid ${hasError ? color.danger : isFocused || open || forceFocus ? color.primary : color.borderDefault}`,
