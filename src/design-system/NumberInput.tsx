...
export interface NumberInputProps {
  ...
  required?: boolean;
  fullWidth?: boolean;
  /** Documentation-only: force the focused border statically (no real focus). */
  forceFocus?: boolean;
}
...
export function NumberInput({
  ...
  const fieldStyle: React.CSSProperties = {
    ...
    alignItems: "stretch",
    border: "1px solid",
    borderColor: hasError ? color.danger : isFocused || forceFocus ? color.primary : color.borderDefault,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: space[2], fontFamily: font.family, ...(fullWidth ? { width: "100%" } : {}) }}>
      {label && (
        <div style={{ display: "flex", alignItems: "center", gap: space[1] }}>
          <label htmlFor={inputId} style={{ fontSize: font.size.xs, color: color.textLabel }}>{label}</label>
