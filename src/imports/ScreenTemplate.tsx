...
function Component3DModelShared({ activeButtons, isViewPage, opacityUpper = 100, opacityLower = 100, visible = true, jaw = "lower" }: { activeButtons: Set<number>; isViewPage: boolean; opacityUpper?: number; opacityLower?: number; visible?: boolean; jaw?: JawType }) {
  // Check if monochrome button (index 0) is active
  const isMonochrome = activeButtons.has(0);
  ...
  return (
    ...
    >
      {jaw === "both" ? (
        // Both arches as two independent meshes → each slider fades its own arch.
        <TeethModel3D
          modelUrl={JAW_MODEL.upper}
          lowerModelUrl={JAW_MODEL.lower}
          width="100%"
          height="100%"
          backgroundColor="transparent"
          showControls={true}
        ...
        <TeethModel3D
          ...
          feedback={isFeedback}
          zoomIn={isMarginLineActive}
          opacity={jaw === "upper" ? opacityUpper : opacityLower}
          heatmap={isHeatmap}
        />
      )}
    </div>
...
export default function ScreenTemplate({ 
  ...
  // Per-arch opacity so the Layers-panel upper/lower sliders fade their own
  // 3D mesh independently (in "both" jaw mode both meshes are shown together).
  const [opacityUpper, setOpacityUpper] = useState<number>(100);
  const [opacityLower, setOpacityLower] = useState<number>(100);
  ...
  // hidden keyboard shortcut (1/2/3/4/5) inside InfoPage.
  const [infoVariant, setInfoVariant] = useState<'classic' | 'wizard' | 'rail' | 'accordion' | 'sticky'>('classic');
  ...
  return (
    <div className="relative size-full" data-name="Screen template" style={{ backgroundColor: '#F4F4F4' }}>
      ...
      <HeaderNavigation
        ...
        scanTabs={externalScanTabs}
        onModelOpacityChange={(jaw, value) => (jaw === 'upper' ? setOpacityUpper(value) : setOpacityLower(value))}
      ...
      {currentPage === 'view' && layout === 'horizontal-top' && (
        <>
          {/* Toolbar — positioned independently so opening the Review panel can
              never reflow or resize it. Its height is fixed (44px button + 8px
              padding = 52px). */}
          <div className="absolute right-[17px] z-10" style={{ top: `${72 + tabsOffset}px` }}>
          ...
          {/* Review panel — opens below the toolbar (52px toolbar + 16px gap)
              and stretches down to 16px above the page bottom. Decoupled from
              the toolbar. */}
          {viewActiveButtons.has(1) && viewActiveButtons.has(3) && combinedPanelMode ? (
            <div className="absolute right-[17px]" style={{ top: `${72 + tabsOffset + 52 + 16}px` }}>
              <CombinedReviewMarginPanel />
            </div>
          ) : (
            viewActiveButtons.has(1) && (
              <div className="absolute right-[17px] w-[320px] max-w-[320px]" style={{ top: `${72 + tabsOffset + 52 + 16}px`, bottom: '16px' }}>
                <CameraNiri />
              </div>
            )
          )}
