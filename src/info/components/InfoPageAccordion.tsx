...
export function InfoPageAccordion({ onContinue, onPatientChange }: Props) {
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", backgroundColor: "var(--ads-bg-page)" }}>
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "32px", display: "flex", flexDirection: "column", gap: "12px" }}>
        {sections.map((s) => (
          <AccordionCard
            key={s.id}
            title={s.title}
...
function AccordionCard({
  ...
  return (
    <section style={{
      ...
      {isOpen && (
        <div style={{ padding: "20px", borderTop: "1px solid var(--ads-border-subtle)" }}>
          {children}
        </div>
      )}
    </section>
